...
if __name__ == '__main__':
    ...
    path = r'E:/Proud/franka-pybullet/src/results/0501/trajectory_logs.txt'
    ...
    spec_traj = _FFT(traj)
    spec_demon = _FFT(demostrantion_end_effector[:, :3])
    print(spec_traj.shape, spec_demon.shape)

    dtw_dis, _ = fastdtw(traj, demostrantion_end_effector[:, :3], dist=euclidean_squared_distance)
    print(dtw_dis)
    mse_dis = np.average((spec_traj - spec_demon) * np.conj(spec_traj - spec_demon))
    print(mse_dis.real)
    print(mse_dis.real - dtw_dis)

    # a1 = np.sum((spec_demon) * np.conj(spec_traj))
    # a2 = np.sum(traj * demostrantion_end_effector[:,:3]) * traj.shape[0] * traj.shape[1]
    # print(a1, a2)