...
'''
z1_robot is api for Unitree robotic arm Z1.
...
class z1_robot:
    def __init__(self, stepsize=1e-3, realtime=0) -> object:
        ...
        p.resetDebugVisualizerCamera(cameraDistance=0.7, cameraYaw=0, cameraPitch=-10, cameraTargetPosition=[0, 0, 0.2])
    ...
    def reset(self):
        self.t = 0.0
        self.control_mode = "torque"
        # self.target_pos = [0.03, 0.0, -0.01, -0.03, -0.03, 0.13]
        self.target_pos = [
            -0.3,
            0.68,
            -1.04,
        ...
        for j in range(self.dof):
            ...
            p.resetJointState(self.robot, j + 1, targetValue=self.target_pos[j])
    ...
    def getJointStates(self):   # 获取关节的位置和速度 返回值：6*1 的关节位置，6*1 的关节速度
    ...
    def solveListKinematics(self, joints_array):
        joints_num = joints_array.shape[1] # joints_array should be N * 6
        if joints_num != self.dof:
            raise Exception('wrong joint trajectory: number of joints is not 6')
        ...
        end_effector_array = np.zeros(shape=(joints_array.shape[0], 7))
        for k in range(joints_array.shape[0]):
            for j in range(joints_array.shape[1]):
                p.resetJointState(self.robot, j, joints_array[k, j])
            link_state = p.getLinkState(self.robot, self.dof) # 修改过，本来是self.dof - 1
