...
class Multi_dimensions_stomp():
    def __init__(self, num_points, args, cost_func, rangelimit_low_list, rangelimit_high_list, 
        vellimit_low_list, vellimit_high_list, acclimit_low_list, acclimit_high_list):
        # 初始化参数
        ...
        if len(self.rangelimit_high_list) != self.dimensions_num or len(self.rangelimit_low_list) != self.dimensions_num or\
            ...
            raise Exception('wrong limit initialize: number of joints limits is not 6')
        else:
    ...
    def calculate_weights(self):
        ...
        state = {"position": np.zeros((self.num_points, self.dimensions_num)), # N * dimensions_num
    ...
    def calculate_delta_noise(self, weights_p):
        proved_noise = np.zeros((self.num_points, self.dimensions_num)) # N * 7
        for j in range(self.dimensions_num):
    ...
    def update_reuse_traj(self, n7_state):
        self.cost_function.Update_state(n7_state)
        ...
        if self.reuse_trajectory < self.args.reuse_num:
            ...
            self.reuse_state["position"][self.reuse_trajectory, :, :] = n7_state["position"]
            self.reuse_state["velocity"][self.reuse_trajectory, :, :] = n7_state["velocity"]
            self.reuse_state["acceleration"][self.reuse_trajectory, :, :] = n7_state["acceleration"]
            ...
            if max_list[index] > cost:
                ...
                self.reuse_state["position"][index, :, :] = n7_state["position"]
                self.reuse_state["velocity"][index, :, :] = n7_state["velocity"]
                self.reuse_state["acceleration"][index, :, :] = n7_state["acceleration"]
